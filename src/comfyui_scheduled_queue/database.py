...
# Busy-wait timeout for SQLite lock contention (milliseconds).
#
# Trade-off analysis
# -----------------
# The aiohttp request thread runs alongside the background scheduler thread.
# Both share a single ``sqlite3.Connection`` (WAL mode lets readers and
# one writer co-exist, but a *single Python-level connection* is not safe
# to use concurrently from two threads — even with ``check_same_thread=
# False`` — without application-level serialisation. The application
# lock below (``_IO_LOCK``) handles that. The ``busy_timeout`` PRAGMA
# covers the residual race when *another process* holds the write lock
# (e.g. a user running ``sqlite3`` CLI during an outage) or when the
# scheduler thread is mid-transaction and a second writer grabs the lock
# just before us.
#
# Two competing concerns:
#   * Too long (e.g. 30s) -> a single aiohttp handler hangs the event
#     loop for 30s. The ComfyUI sidebar freezes. This was the original
#     bug report.
#   * Too short (e.g. 100ms) -> an honest lock contention immediately
#     surfaces as a "database is locked" exception that the handler has
#     to swallow and retry. We end up doing the retry work the busy
#     timeout was supposed to do for us, but worse (with extra round-
#     trips through the asyncio task layer).
#
# Empirically, a scheduler tick runs in <50ms (see scheduler.py:IDLE
# path). The longest single write in the codebase is ``mark_done``,
# which migrates a row from ``scheduled_jobs`` to ``job_history`` —
# ~5ms on a fresh DB. So 1 second comfortably covers the realistic
# worst case (10 scheduler ticks in a row) without ever letting an
# aiohttp handler stall beyond a perceived "loading" spinner.
...
class ScheduledQueueDB:
    # Application-level mutex that serialises every method on this class.
    #
    # SQLite's own locking layer (busy_timeout + WAL mode) handles *cross-
    # process* contention (e.g. a human running the ``sqlite3`` CLI on
    # the same file), but a single ``sqlite3.Connection`` is NOT safe for
    # concurrent use from multiple Python threads even with
    # ``check_same_thread=False`` — interleaving an INSERT on one thread
    # with a SELECT on another can corrupt the cursor's internal state
    # and raise ``ProgrammingError: SQLite objects created in a thread
    # can only be used in that same thread`` after the first lock-cycle.
    # The aiohttp handler thread and the background scheduler thread
    # both share this connection, so we need a single in-process mutex
    # to keep them from trampling each other. ``RLock`` (not ``Lock``)
    # so re-entrant helpers (e.g. ``add_job`` calling ``update_job``)
    # don't deadlock on themselves.
    #
    # Holding the lock across every public call costs ~5µs per call but
    # is the only way to make the busy_timeout PRAGMA actually useful —
    # without it, two threads racing on the same connection raise before
    # the busy handler ever fires.
    ...
    def __init__(self, db_path: str | None = None):
        ...
        # Keep the PRAGMA inline with the constant above so future
        # tuning happens in one place. The 1s ceiling is the upper
        # bound on how long *another process* can stall a connection
        # mid-query; the in-process lock keeps us from ever needing it.
    ...
    def add_job(self, payload, scheduled_at, priority=100, note=None, client_id=None, auto_retry=0, workflow_title=None):
        # The application lock wraps the whole method (including the
        # SELECT for the next queue_order) so the scheduler thread's
        # claim_next_due_job can never see a partial state where the
        # row has been INSERTed but queue_order has not yet been picked.
        # The ``with self._conn:`` line then opens the SQLite write
        # transaction.
        with self._IO_LOCK:
    ...
    def get_job(self, job_id):
        # Read paths still go through the lock — a concurrent
        # ``claim_next_due_job`` could otherwise DELETE the row mid-read
        # and leave us with a half-populated sqlite3.Row.
        with self._IO_LOCK:
            return _with_duration(self._conn.execute("SELECT * FROM scheduled_jobs WHERE id=?",(job_id,)).fetchone())
    ...
    def get_job_with_outputs(self, job_id):
        ...
        """
        # RLock makes the re-entrant call to ``self.get_job`` safe even
        # though both paths take the same lock. Without RLock we'd
        # deadlock here.
        with self._IO_LOCK:
    ...
    def repeat_job(self, job_id, priority=100, scheduled_at=None):
        ...
        """
        # We hold the lock for the entire lookup AND the add_job() that
        # follows so another concurrent thread can't squeeze an INSERT
        # between the SELECT (which picks queue_order) and the actual
        # add. add_job() re-acquires the same RLock safely.
        with self._IO_LOCK:
    ...
    def reorder_job(self, job_id, direction):
        # The whole reorder is one logical step (read neighbour, swap
        # both queue_orders). Splitting the lock between the two UPDATEs
        # would let another thread observe the half-swapped state, so we
        # hold the lock for the whole multi-statement transaction.
        with self._IO_LOCK:
    ...
    def claim_next_due_job(self):
        # The scheduler thread runs this every tick; serialising it is the
        # whole reason _IO_LOCK exists. Without the lock, a concurrent
        # list_jobs() reading the queue could see the row in its old
        # ('scheduled') state after we've already UPDATEd it to
        # 'dispatched' — causing a double-dispatch.
        with self._IO_LOCK:
            now=time.time()
            with self._conn:
    ...
    def _finish(self, job_id, status, prompt_id=None, outputs=None, error=None):
        # _finish does a SELECT + INSERT + DELETE — all three must hold the
        # lock so the side-car /history writes never interleave with a
        # concurrent repeat_job() reading the same job_id.
        with self._IO_LOCK:
            now=time.time()
            with self._conn:
            ...
            return True
    def mark_done(self,job_id,prompt_id=None,outputs=None):
        with self._IO_LOCK: return self._finish(job_id,'done',prompt_id,outputs)
    def mark_failed(self,job_id,error):
        with self._IO_LOCK: return self._finish(job_id,'failed',error=error)
    ...
    def increment_retry(self,job_id):
        # The UPDATE + SELECT must be inside one lock acquisition so
        # the return value reflects the post-increment count rather
        # than some interleaved state.
        with self._IO_LOCK:
